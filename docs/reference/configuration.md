...
* `blockstore.s3.endpoint` `(string : )` - An optional endpoint URL (hostname only or fully qualified URI) that overrides the default generated endpoint for a client. Required for 
o setup.
* `blockstore.s3.force_path_style` `(boolean : false)` - Set this to `true` to force the request to use [path style addressing](https://docs.aws.amazon.com/AmazonS3/latest/dev/VirtualHosting.html#path-style-access) (eg. when using Minio)
